...
function requestThermalCalculation(T) {
    if (thermalWorker) {
        thermalWorker.postMessage({ type: 'CALCULATE_DAYS', payload: { T, T0, TH, K } });
    } else {
...
function updAll() { requestThermalCalculation(curTemp); }
...
function drawChart() {
    ...
    ctx.strokeStyle = 'rgba(255,255,255,.04)'; ctx.lineWidth = 1; ctx.fillStyle = 'rgba(154,169,182,.4)'; ctx.font = '9px Tajawal'; ctx.textAlign = 'right';
    for (let i = 0; i <= 5; i++) {
    ...
    data.forEach((d, i) => {
        ...
        ctx.fillStyle = 'rgba(154,169,182,.4)'; ctx.fillText(d.m, x, H - pad.b + 16);
        ctx.fillStyle = tClr(d.t); ctx.font = '8px Tajawal'; ctx.fillText(d.t + '°', x, H - pad.b + 30);
    });
    ...
    data.forEach((d, i) => {
        ...
        if (d.days > 0) { 
            ctx.fillStyle = clr; drawRoundedRect(ctx, x - bw / 2, by, bw, bh, 4); ctx.fill(); 
            ctx.fillStyle = '#fff'; ctx.font = 'bold 9px Cairo'; ctx.fillText(d.days, x, by - 4); 
        }
    });
    ...
    data.forEach((d, i) => {
        const x = pad.l + (i + .5) * (cW / 12);
        const y = pad.t + cH - (d.t / maxTemp) * cH;
        ctx.beginPath(); ctx.arc(x, y, 4.5, 0, Math.PI * 2); ctx.fillStyle = tClr(d.t); ctx.fill();
        ctx.strokeStyle = '#fff'; ctx.lineWidth = 1.5; ctx.stroke();
    });
    
    [[8, 'rgba(100,180,200,.5)', 'العتبة 8°م'], [30, 'rgba(46,204,113,.5)', 'المثلى 30°م']].forEach(([t, cl, lb]) => {
        const y = pad.t + cH - (t / maxTemp) * cH;
        ctx.beginPath(); ctx.strokeStyle = cl; ctx.lineWidth = 1; ctx.setLineDash([5, 5]);
        ctx.moveTo(pad.l, y); ctx.lineTo(pad.l + cW, y); ctx.stroke(); ctx.setLineDash([]);
        ctx.fillStyle = cl; ctx.font = '8px Tajawal'; ctx.textAlign = 'left'; ctx.fillText(lb, pad.l + 4, y - 5);
    });
    
    data.forEach((d, i) => {
        const x = pad.l + (i + .5) * (cW / 12);
        ctx.fillStyle = d.gen >= 2 ? 'rgba(231,76,60,.6)' : d.gen >= 1 ? 'rgba(243,156,18,.5)' : 'rgba(154,169,182,.3)';
        ctx.font = 'bold 8px Cairo'; ctx.textAlign = 'center'; ctx.fillText(d.gen + ' جيل', x, H - pad.b + 44);
    });
...
function buildStages() {
    ...
    stagesData.forEach((s, i) => {
        ...
        c.className = 'sc2'; c.style.setProperty('--sc2', s.color); c.dataset.i = i; c.tabIndex = 0;
        c.setAttribute('role', 'listitem'); c.setAttribute('aria-label', `${s.name} - ${d[s.id] > 0 ? d[s.id] + ' يوم' : 'لا تطور'}`);
...
function toggleA() {
    ...
    if (isAuto) { clearInterval(autoInt); isAuto = false; b.innerHTML = '<i class="fas fa-play" aria-hidden="true"></i><span>تشغيل تلقائي</span>'; } 
    else { isAuto = true; b.innerHTML = '<i class="fas fa-pause" aria-hidden="true"></i><span>إيقاف</span>'; let s = 0; selS(s); autoInt = setInterval(() => { s = (s + 1) % stagesData.length; selS(s); }, 3500); }
}
...
function populateTable() {
    ...
    rows.forEach(row => {
        ...
        row.data.forEach((val, i) => {
            ...
            else if (row.type === 'hum') td.innerHTML = `<span class="circle-badge" style="background:${getHumidityColor(val)};">${val}</span>`;
            else if (row.type === 'act') {
                const level = getActivityLevel(val); const color = getActivityColor(level);
            ...
            else if (row.type === 'trap') td.textContent = val;
...
function buildPlanCards() {
    ...
    planCardsData.forEach(p => {
        ...
        card.className = 'plan-card'; card.id = 'card-' + p.id; card.style.borderRight = `4px solid ${p.color}`; card.style.padding = '1rem';
...
function showPlanCard(period, btn) {
    document.querySelectorAll('#planFilterCol .filter-btn').forEach(b => { b.classList.remove('active'); b.setAttribute('aria-pressed', 'false'); });
    btn.classList.add('active'); btn.setAttribute('aria-pressed', 'true');
    document.querySelectorAll('.plan-card').forEach(c => { c.classList.add('hidden-card'); c.style.opacity = '0'; c.style.transform = 'translateX(-20px)'; });
    const target = document.getElementById('card-' + period);
    if (target) { target.classList.remove('hidden-card'); requestAnimationFrame(() => { target.style.opacity = '1'; target.style.transform = 'translateX(0)'; }); }
}

function filterSeason(season, btn) {
    document.querySelectorAll('.filter-bar .filter-btn').forEach(b => { b.classList.remove('active'); b.setAttribute('aria-pressed', 'false'); });
    btn.classList.add('active'); btn.setAttribute('aria-pressed', 'true');
    ...
    document.querySelectorAll('th[data-month]').forEach(th => { const month = parseInt(th.getAttribute('data-month')); th.classList.toggle('hidden-col', !visible.includes(month)); });
    document.querySelectorAll('td[data-month]').forEach(td => { const month = parseInt(td.getAttribute('data-month')); td.classList.toggle('hidden-col', !visible.includes(month)); });
}
...
function buildSources() {
    ...
    setTimeout(() => {
        if (sourcesData.length === 0) { showEmptyState('sourcesBox', 'لا توجد مصادر متاحة حالياً.'); return; }
        const fragment = document.createDocumentFragment();
        sourcesData.forEach(s => {
            const card = document.createElement('div');
            card.className = 'source-card'; card.setAttribute('role', 'listitem');
...
function buildSpreadSection() {
    ...
    reasons.forEach(r => {
        const card = document.createElement('div');
        card.className = 'bio-card'; card.setAttribute('role', 'listitem'); card.dataset.category = r.category;
        card.innerHTML = `<div class="bio-header" onclick="toggleAccordion(this.closest('.bio-card'), 'spreadAccordion')" tabindex="0" aria-label="${r.title}"><span style="font-size: 2rem;">${r.icon}</span><div style="flex:1"><h3 style="font-size:1.05rem; color:#fff;">${r.title}</h3><span style="font-size:0.75rem; color: var(--plan-accent);">${r.type}</span></div><i class="fas fa-chevron-down" aria-hidden="true"></i></div><div class="bio-body"><p>${r.description}</p><h4>🔑 الأثر</h4><p>${r.impact}</p></div>`;
        fragment.appendChild(card);
    });
...
function buildEconomicSection() {
    ...
    if (statsContainer) {
        ...
        stats.forEach(s => {
            ...
            stat.className = 'econ-stat'; stat.setAttribute('role', 'listitem');
    ...
    if (cardsContainer) {
        ...
        cards.forEach(c => {
            const card = document.createElement('div');
            card.className = 'bio-card'; card.setAttribute('role', 'listitem'); card.dataset.category = c.category;
            card.innerHTML = `<div class="bio-header" onclick="toggleAccordion(this.closest('.bio-card'), 'econAccordion')" tabindex="0"><span style="font-size: 2rem;">${c.icon}</span><div style="flex:1"><h3 style="font-size:1.05rem; color:#fff;">${c.title}</h3><span style="font-size:0.75rem; color: var(--plan-accent);">${c.type}</span></div><i class="fas fa-chevron-down" aria-hidden="true"></i></div><div class="bio-body"><p>${c.description}</p><h4>💰 الأثر المالي</h4><p>${c.financialImpact}</p></div>`;
            fragment.appendChild(card);
        });
...
function buildIPMSection() {
    ...
    tabs.forEach((tab, i) => { html += `<button class="ipm-tab ${i === 0 ? 'active' : ''}" data-tab="${tab.id}" role="tab" aria-selected="${i === 0 ? 'true' : 'false'}" tabindex="0">${tab.title}</button>`; });
    ...
    tabs.forEach((tab, i) => {
        ...
        if (panel.warning) html += `<div style="padding:1rem;background:rgba(231,76,60,0.08);border:1px solid rgba(231,76,60,0.3);border-radius:var(--radius-sm);margin-bottom:1.5rem;display:flex;gap:1rem;align-items:flex-start;"><span style="font-size:1.5rem;">⚠️</span><p style="font-size:0.9rem;color:var(--text2);line-height:1.7;"><strong style="color:var(--accent);">تحذير:</strong> ${panel.warning}</p></div>`;
        if (panel.intro) html += `<div style="margin-bottom:1.5rem;padding:1.2rem;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-sm);"><h3 style="color:#fff;font-size:1.2rem;margin-bottom:0.8rem;">مبادئ إدارة مقاومة المبيدات (IRM)</h3><p style="color:var(--text2);font-size:0.9rem;line-height:1.8;">${panel.intro}</p></div>`;
        if (panel.cards) {
            html += '<div class="ipm-grid">';
            panel.cards.forEach(card => { html += `<div class="ipm-card"><div class="ipm-card-icon">${card.icon}</div><h4>${card.title}</h4><p>${card.description}</p><span class="ipm-tag ${card.tagClass}">${card.tag}</span></div>`; });
            html += '</div>';
        }
        if (panel.instructions) html += `<div style="background:rgba(243,156,18,0.08);border:1px solid rgba(243,156,18,0.3);border-radius:var(--radius-sm);padding:1.2rem;margin-top:1.5rem;"><h4 style="color:var(--amber);margin-bottom:0.5rem;">⚠️ إرشادات هامة لاستخدام المبيدات</h4><ul style="list-style:none;padding:0;font-size:0.85rem;color:var(--text2);line-height:2;">${panel.instructions.map(inst => `<li>✓ ${inst}</li>`).join('')}</ul></div>`;
        if (panel.rotationSchedule) html += `<div style="margin-top:1.5rem;padding:1.2rem;background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-sm);"><h4 style="color:var(--plan-accent);margin-bottom:0.8rem;">🗓️ جدول تناوب IRAC المقترح</h4><p style="color:var(--text2);font-size:0.9rem;line-height:1.9;">${panel.rotationSchedule}</p></div>`;
        html += '</div>';
    });
    html += '</div>';
    container.innerHTML = html;
    setTimeout(() => {
        ...
        if (tabsContainer) {
            tabsContainer.addEventListener('click', function(e) {
                const tab = e.target.closest('.ipm-tab'); if (!tab) return;
                const target = tab.dataset.tab;
                document.querySelectorAll('.ipm-tab').forEach(t => { t.classList.remove('active'); t.setAttribute('aria-selected', 'false'); });
...
function buildFAQSection() {
    ...
    faqs.forEach(f => {
        ...
        item.className = 'faq-item'; item.setAttribute('role', 'listitem');
        item.innerHTML = `<div class="faq-question" onclick="toggleFAQ(this)" tabindex="0" aria-expanded="false" role="button"><span>${f.question}</span><i class="fas fa-chevron-down faq-icon" aria-hidden="true"></i></div><div class="faq-answer" role="region">${f.answer}</div>`;
...
function buildResistanceSection() {
    ...
    data.forEach(r => {
        ...
        const card = document.createElement('div');
        card.className = 'resistance-card'; card.setAttribute('role', 'listitem');
        card.innerHTML = `<h4>${r.pesticide}</h4><p style="font-size:0.8rem;color:var(--text2)">${r.example}</p><div class="resistance-level ${levelClass}">${r.levelText}</div>`;
        fragment.appendChild(card);
    });
    container.appendChild(fragment);
}
...
function toggleFAQ(el) {
    ...
    document.querySelectorAll('.faq-item').forEach(x => { x.classList.remove('open'); const q = x.querySelector('.faq-question'); if(q) q.setAttribute('aria-expanded', 'false'); });
...
const badgeMap = {
    'preventive': { text: '🛡️ وقائي فقط', class: 'badge-blue' }, 'curative': { text: '💊 علاجي فقط', class: 'badge-amber' },
    'preventive-curative': { text: '🛡️💊 وقائي وعلاجي', class: 'badge-green' }, 'heat-tolerant': { text: '🌡️ متحمل للحرارة', class: 'badge-green' },
    'egypt-native': { text: '🇪🇬 متوطن في مصر', class: 'badge-purple' }, 'pesticide-sensitive': { text: '⚠️ حساس للمبيدات', class: 'badge-red' },
    'bio-safe': { text: '✅ آمن مع المبيدات الحيوية', class: 'badge-green' }, 'needs-humidity': { text: '💧 يحتاج رطوبة', class: 'badge-amber' },
    'needs-high-humidity': { text: '💧 يحتاج رطوبة عالية', class: 'badge-red' }, 'medium-heat': { text: '🌡️ تحمل حراري متوسط', class: 'badge-amber' },
    'good-heat': { text: '🌡️ تحمل حراري جيد', class: 'badge-green' }, 'sun-sensitive': { text: '☀️ حساس للشمس', class: 'badge-red' },
...
const categoryMap = {
    'egg-parasitoid': { name: '🐝 طفيلات البيض' }, 'larval-parasitoid': { name: '🐝 طفيلات اليرقات' },
...
function renderBioCategoryFilter() {
    ...
    if (categories.length > 0) { renderBioCards(categories[0]); }
}
...
function renderBioCards(filter = 'egg-parasitoid') {
    ...
    setTimeout(() => {
        ...
        if (filtered.length === 0) { showEmptyState('bioCardsContainer', 'لا يوجد أعداء حيويين في هذه الفئة حالياً.'); return; }
        container.innerHTML = '';
        const fragment = document.createDocumentFragment();
        ...
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                if (!renderedBioCards.has(agentId)) {
                    ...
                    if (agent) { renderBioCardFullContent(card, agent); renderedBioCards.add(agentId); }
                }
...
function renderBioCardFullContent(card, agent) {
    ...
    html += '</div><div class="bio-badges">';
    agent.badges.forEach(b => { const bd = badgeMap[b]; if (bd) html += `<span class="bio-badge ${bd.class}">${bd.text}</span>`; });
    html += '</div><div class="bio-card-footer">اقرأ التفاصيل الكاملة <i class="fas fa-arrow-left" aria-hidden="true"></i></div>';
    card.innerHTML = html;
}
...
function renderBioModals() {
    ...
    bioAgentsData.forEach(agent => {
        ...
        modal.className = 'bio-modal'; modal.id = `modal-${agent.id}`;
        modal.setAttribute('role', 'dialog'); modal.setAttribute('aria-modal', 'true'); modal.setAttribute('aria-label', agent.scientificName);
        ...
        const overview = `<div class="tab-content active" id="tab-${agent.id}-overview" role="tabpanel"><div class="modal-body-content"><h4>🔬 التصنيف العلمي</h4><table class="info-table"><tr><td>الرتبة</td><td>${agent.classification.order}</td></tr><tr><td>الفصيلة</td><td>${agent.classification.family}</td></tr><tr><td>الجنس</td><td>${agent.classification.genus}</td></tr><tr><td>النوع</td><td><strong>${agent.classification.species}</strong></td></tr></table><h4>🧬 النوع الحيوي وطريقة العمل</h4><p>${agent.bioType}</p><h4>👁️ الوصف المورفولوجي</h4><table class="info-table"><tr><td>الحشرة الكاملة</td><td>${agent.morphology.adult}</td></tr><tr><td>البيضة</td><td>${agent.morphology.egg}</td></tr><tr><td>اليرقة</td><td>${agent.morphology.larva}</td></tr><tr><td>العذراء</td><td>${agent.morphology.pupa}</td></tr></table><h4>📊 الأهمية في المكافحة</h4><div class="importance-grid"><div class="importance-item ${agent.importance.egg}"><span>🥚 مكافحة البيض</span><span class="importance-level ${agent.importance.egg}">${importanceText[agent.importance.egg]}</span></div><div class="importance-item ${agent.importance.larvae}"><span>🐛 مكافحة اليرقات</span><span class="importance-level ${agent.importance.larvae}">${importanceText[agent.importance.larvae]}</span></div><div class="importance-item ${agent.importance.pupae}"><span>🫘 مكافحة العذارى</span><span class="importance-level ${agent.importance.pupae}">${importanceText[agent.importance.pupae]}</span></div><div class="importance-item ${agent.importance.adult}"><span>🦋 مكافحة الكاملة</span><span class="importance-level ${agent.importance.adult}">${importanceText[agent.importance.adult]}</span></div></div></div></div>`;
...
function switchTab(agentId, tabName, event) {
    ...
    modal.querySelectorAll('.tab-btn').forEach(tb => { tb.classList.remove('active'); tb.setAttribute('aria-selected', 'false'); });
    const target = document.getElementById(`tab-${agentId}-${tabName}`);
    if (target) target.classList.add('active');
    if (event) { const btn = event.target.closest('.tab-btn'); if (btn) { btn.classList.add('active'); btn.setAttribute('aria-selected', 'true'); } }
}

function filterBioByCategory(category, btn) {
    document.querySelectorAll('.bio-filter-btn').forEach(b => { b.classList.remove('active'); b.setAttribute('aria-pressed', 'false'); });
    btn.classList.add('active'); btn.setAttribute('aria-pressed', 'true');
...
document.addEventListener('keydown', function(e) {
    ...
    if (e.key === 'Enter' || e.key === ' ') {
        ...
        if (target.classList.contains('bio-header') || target.classList.contains('faq-question') || target.classList.contains('filter-btn') || target.classList.contains('ipm-tab') || target.classList.contains('bio-filter-btn') || target.classList.contains('drawer-nav-item') || target.classList.contains('drawer-header-block') || target.getAttribute('tabindex') === '0') {
...
// ============================================
// Progress Bar, Double-Tap Zoom, Pull-to-Refresh
// ============================================
...
function updateProgressBar() {
    ...
    if (scrollTop > 100) { progressBar.classList.add('visible'); } else { progressBar.classList.remove('visible'); }
}
...
function handleDoubleTap(element) {
    ...
    if (tapLength < 300 && tapLength > 0) { element.classList.toggle('zoomed'); }
    lastTap = currentTime;
}
...
function initPullToRefresh() {
    ...
    document.addEventListener('touchstart', (e) => {
        if (window.pageYOffset === 0) { pullStartY = e.touches[0].clientY; isPulling = true; }
    }, { passive: true });
    ...
    document.addEventListener('touchend', () => {
        if (!isPulling) return;
        isPulling = false;
        if (pullDistance > PULL_THRESHOLD) { performRefresh(pullIndicator); } else { resetPullIndicator(pullIndicator); }
        pullDistance = 0;
    }, { passive: true });
}
...
function resetPullIndicator(pullIndicator) {
    pullIndicator.style.transform = 'translate(-50%, -150%)';
    pullIndicator.style.opacity = '0';
    pullIndicator.querySelector('.pull-text').textContent = 'اسحب للتحديث';
    pullIndicator.querySelector('i').style.animation = '';
    pullIndicator.querySelector('i').style.transform = 'rotate(0deg)';
}
